...
class KRCrossSessionEnvironment_GPU():
    '''
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument('--response_prob_multi', type=float, default=1.0,
                            help='multiplier for response probability')
        parser.add_argument('--response_prob_high', type=float, default=1.1,
                            help='highest multiplier for response probability')
        parser.add_argument('--response_prob_low', type=float, default=0.9,
                            help='lowest multiplier for response probability')
        parser.add_argument('--random_retention', action='store_true', 
                            help='True if using random user retention on bias')
        parser.add_argument('--im_retention', action='store_true', 
                            help='True if using the retention model trained wit immediate response')
        return parser
    ...
    def __init__(self, args):
        ...
        urrm_stats, urrm_model, urrm_args = self.get_user_model(args.urrm_log_path, args.device, 
                                                                from_load = not args.random_retention)
        ...
        self.im_retention = args.im_retention
        self.response_prob_multi = args.response_prob_multi
        self.response_prob_high = args.response_prob_high
        self.response_prob_low  = args.response_prob_low
        self.tot_steps = 0
    ...
    def step(self, step_dict):
        ...
        self.tot_steps += 1
        response_prob_multi = 1
        if self.tot_steps % 1000 == 0:
            response_prob_multi = random.uniform(self.response_prob_low, self.response_prob_high)
        ...
        with torch.no_grad():
            ...
            selected_scores = torch.clip(selected_scores * response_prob_multi, 0, 1)
            # (B, slate_size, n_feedback)
            ...
            retention_out_dict = self.retention_model({'user_id': self.current_observation['user_profile']['user_id'], 
                                                    #    'sess_encoding': new_gt_state})
                                                         'history_response': update_info['updated_observation']['user_history']})
            # (B, max_return_day)
