...
class GFN4Retention_wif_teacher_computer_resources(BaseRLAgent):
    @staticmethod
    ...
    def setup_monitors(self):
        ...
        self.training_history['peak_gpu_memory_mb'] = []
    ...
    def step_train(self):
        ...
        loss, discrepancies, student_reward = self.get_gfn_loss(observation, policy_output, user_feedback, done_mask, next_observation, is_teacher=False)
        ...
        self.training_history['step_time'].append(step_time)
        if torch.cuda.is_available():
            peak_mem = torch.cuda.max_memory_allocated() / (1024 ** 2)
            self.training_history['peak_gpu_memory_mb'].append(peak_mem)
    ...
    def action_after_train(self):
        '''
        Print training summary: average step time and peak GPU memory.
        '''
        if len(self.training_history['step_time']) == 0:
            print("No training steps recorded.")
            return

        avg_step_time = sum(self.training_history['step_time']) / len(self.training_history['step_time'])
        total_time = sum(self.training_history['step_time'])
        n_steps = len(self.training_history['step_time'])
        print(f"\n========== Training Summary ==========")
        print(f"Total training steps: {n_steps}")
        print(f"Total training time: {total_time:.2f} seconds")
        print(f"Average time per step: {avg_step_time * 1000:.2f} ms")

        if torch.cuda.is_available() and len(self.training_history['peak_gpu_memory_mb']) > 0:
            max_gpu_mem = max(self.training_history['peak_gpu_memory_mb'])
            print(f"Peak GPU memory allocated: {max_gpu_mem:.2f} MB")
        else:
            print("GPU memory tracking not available (CUDA not used).")
        print(f"======================================\n")
